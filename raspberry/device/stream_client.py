...
Le Pi streame le PCM 16k au backend (qui relaie vers ElevenLabs Scribe), reçoit les
transcripts partiels au fil de l'eau, et — à l'EOT (décidé localement par Smart Turn)
— la réponse texte + le MP3 TTS streamé. Synchrone (websocket-client) + thread
receveur → file. AUCUNE clé sur l'appareil : auth par DEVICE_TOKEN.

Messages reçus (kind) : "partial"/"committed" (transcript), "response" (texte LLM),
"audio" (frames MP3 binaires), "audio_end", "final", "error", "closed".
"""
import json
import queue
import logging
